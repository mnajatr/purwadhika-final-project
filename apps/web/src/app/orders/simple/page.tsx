      ...
      <div className="mb-6">
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`px-4 py-2 rounded ${currentStatus === "" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
          >
            All ({orders.length})
          </button>
          <button
            onClick={() => handleStatusClick("PENDING_PAYMENT")}
            className={`px-4 py-2 rounded ${currentStatus === "PENDING_PAYMENT" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
          >
            Pending Payment
          </button>
          <button
            onClick={() => handleStatusClick("PAYMENT_REVIEW")}
            className={`px-4 py-2 rounded ${currentStatus === "PAYMENT_REVIEW" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
          >
            Payment Review
          </button>
          <button
            onClick={() => handleStatusClick("PROCESSING")}
            className={`px-4 py-2 rounded ${currentStatus === "PROCESSING" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
          >
            Processing
          </button>
          <button
            onClick={() => handleStatusClick("CANCELLED")}
            className={`px-4 py-2 rounded ${currentStatus === "CANCELLED" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
          >
            Cancelled
          </button>
        </div>
