...
type ResolveResp = { success: boolean; nearestStore: { id: number; name?: string } | null; message?: string };
...
export default function AddressPicker({ userId }: { userId?: number }) {
  ...
  const handleSelectCb = React.useCallback(
    async (a: Addr, callResolve = true) => {
      ...
      try {
        ...
        const storedUserId =
          typeof window !== "undefined" ? sessionStorage.getItem("checkout:userId") : null;
    ...
    const resp = await apiClient.get<ResolveResp>(`/stores/resolve?userId=${uid}&addressId=${a.id}`);
  ...
  type RespShapeA = { data: { nearestStore: { id: number; name?: string } | null; message?: string } };
  type RespShapeB = { nearestStore: { id: number; name?: string } | null; message?: string };
      const r = resp as unknown as RespShapeA | RespShapeB;
      const store = (r as RespShapeA).data?.nearestStore ?? (r as RespShapeB).nearestStore ?? null;
      // DEBUG: log resolve response and store
      try {
        console.debug("/stores/resolve response:", r, "resolved store:", store);
      } catch {}
      ...
      try {
        if (typeof window !== "undefined") {
          if (store?.id) localStorage.setItem("nearestStoreId", String(store.id));
          else localStorage.removeItem("nearestStoreId");
          if (store?.name) localStorage.setItem("nearestStoreName", store.name);
          else localStorage.removeItem("nearestStoreName");
          console.debug("locationStore updated: nearestStoreId=", localStorage.getItem("nearestStoreId"), "nearestStoreName=", localStorage.getItem("nearestStoreName"));
        }
      } catch {}
      } catch (err) {
    ...
    [setActiveAddress, setNearestStoreId, setNearestStoreName, userId, queryClient]
  );
  ...
  return (
    <div className="space-y-3">
      {addrs && addrs.length > 0 ? (
        addrs.map((a) => (
          <label key={a.id} className="block cursor-pointer p-3 border rounded-md">
            <input
              type="radio"
              name="address"
            ...
            <div className="text-xs">{a.city}, {a.province} {a.postalCode}</div>
          </label>
        ))
      ) : (
        <div>No saved addresses. Add one in your profile or proceed with default address.</div>
      )}
    </div>
  );
}
